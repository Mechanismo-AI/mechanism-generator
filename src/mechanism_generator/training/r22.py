#!/usr/bin/env python3
"""
V5.3-R2.2 Constrained Transmission Training

Constrained-transmission fork of V5.3-R2.1_TargetWeightedTransmission.

R2.2 keeps the stable direct circle-circle geometry and coherent recovery
system, but changes how the transmission objective is communicated:

  * a frozen R2 teacher supplies a per-target path-accuracy budget;
  * raw path distance is logged, but optimization penalizes only path error
    beyond the teacher distance plus a configurable tolerance;
  * target-to-frame soft assignment is recomputed each pass and detached, so
    transmission gradients change linkage geometry rather than gaming the
    correspondence weights;
  * the target transmission hinge is strengthened;
  * the global anti-toggle term uses the worst fraction of crank frames rather
    than being diluted by an all-frame mean;
  * the scheduler is held during the objective ramp, then monitors a
    feasibility-aware transmission metric; and
  * separate best-path, best-composite, and best-feasible-transmission
    checkpoints are retained.

The hard nearest-point path measurement is preserved for reporting and budget
construction. Ordered targets, velocities, accelerations, and soft path
correspondence remain later experiments.
"""
...
EXPERIMENT_MODE = "R2.2"
IS_R1 = True
VARIANT = "V5.3-R2.2_ConstrainedTransmission"
PARENT_VARIANT = "V5.3-R2.1_TargetWeightedTransmission"
BASELINE_SOURCE_SHA256 = "f02dd296743bf8da9c17681a76eb0bedaa924db13fb22eefc5e665ef243105d7"
PARENT_SOURCE_SHA256 = "77d1b82986ef641453ca351a154ffd5d56cef07f0041a08fd38903d572d12220"
...
parser = argparse.ArgumentParser(
    description=f"{VARIANT}: teacher-constrained transmission experiment",
...
parser.add_argument("--initial_lr", type=float, default=2.5e-5, help="Initial AdamW learning rate")
parser.add_argument(
    "--initialize_from_model",
    type=str,
    default=None,
    help="Load student model weights only, then start a fresh R2.2 optimizer/run",
)
parser.add_argument(
    "--teacher_model",
    type=str,
...
# theta4 = alpha - phi assembly mode. R2.2 adds teacher-constrained transmission
# training while leaving the corrected geometry unchanged.
...
R21_EXTRA_COMPONENT_KEYS = R22_PATH_COMPONENT_KEYS + R21_TRANSMISSION_COMPONENT_KEYS + R2_GEOMETRY_COMPONENT_KEYS
...
# R2.2 retains the R1.1 instrumentation and hard-failure thresholds.  A dead-gradient recovery
...
def log_rollback_event(
    ...
    append_csv(ROLLBACK_LOG, {
        ...
        "lr_before": lr_before,
        "lr_after": lr_after,
        # R2.2 preserves the explicit R1.1 field ordering.
...
def simulate_four_bar_coupler_point_torch_batch(
    ...
    diagnostics = {
        ...
        # Internal full-resolution tensor used by the R2.2 constrained transmission loss.
...
def compute_mechanism_loss_batch(
    ...
    """Teacher-constrained path objective plus detached transmission training.

    Raw student path distance is always reported. Optimization pays a path
    penalty only when a target is farther away than the frozen teacher's
    corresponding distance plus PATH_TOLERANCE.
    """
    l2, l3, l4 = predicted_params[:, 0], predicted_params[:, 1], predicted_params[:, 2]
    l1 = torch.full_like(l2, L1_FIXED_VALUE)
    ...
    if teacher_target_distances is None:
        teacher_target_distances = compute_teacher_target_distances(
            target_points, steps
        )
    ...
    point_path_guard_penalty = (
        POINT_PATH_GUARD_WEIGHT * point_path_guard_penalty_raw
    )
    ...
    soft_weights = torch.softmax(
        -target_sqd.detach() / temperature, dim=2
    ).detach()
    ...
    components = {
        ...
        "path_max_excess": torch.maximum(
            path_excess.max(), point_path_excess.max()
        ).item(),
...
def restore_checkpoint_r1(path: Path | str) -> Tuple[bool, Optional[Dict[str, Any]]]:
    ...
    try:
        ...
        if "optimizer_state_dict" not in checkpoint:
            raise KeyError("optimizer_state_dict is required for coherent R2.2 recovery")
        optimizer.load_state_dict(checkpoint["optimizer_state_dict"])
        ...
        if checkpoint.get("scheduler_state_dict") is not None:
            ...
            print("[WARN] R2.2 checkpoint lacked scheduler state; scheduler recreated.")
...
def rollback_and_recover_r1(
    ...
    if selected is None:
        ...
        print(
            f"[RECOVERY] R2.2 found no eligible checkpoint ({reason}); "
    ...
    if not loaded or metadata is None:
        ...
        print(
            f"[RECOVERY] R2.2 restore failed; applied in-place LR backoff "
    ...
    log_event(
        ...
        lr_after=recovery_lr,
        details=(
            "R2.2 coherent restore: AdamW moments, scheduler, data, sampler/RNG "
            f"restored; no fresh data; plateau cooldown={grace_counter}"
        ),
    )
    print(
        f"[RECOVERY] R2.2 restored {selected['source']} checkpoint "
...
def log_epoch_metrics(
    ...
    row: Dict[str, Any] = {
        ...
        "scheduler_active": epoch_number > SCHEDULER_START_EPOCH,
        "optimizer_id": id(optimizer),
        "scheduler_optimizer_id": id(getattr(scheduler, "optimizer", None)),
        "scheduler_attached": scheduler_is_attached(),
...
if teacher_source_argument is None:
    raise ValueError(
        "R2.2 requires --teacher_model or --initialize_from_model so a frozen "
...
if teacher_source_variant not in ("V5.3-R2_GeometryFix", VARIANT):
    print(
        "[WARN] The recommended R2.2 teacher is the frozen "
...
if args.initialize_from_model:
    ...
    print(
        f"[INFO] Initialized R2.2 student weights from {initialization_path} "
...
for epoch in range(start_epoch, NUM_EPOCHS):
    ...
    if hard_failure_reason is not None:
        ...
        if recovered:
            continue
        print("[FATAL] R2.2 could not recover from a hard numerical failure.")
        break
    ...
    # R2.2 retains hard-failure protection that bypasses cooldown and all ordinary plateau
        ...
        if epoch < SCHEDULER_START_EPOCH:
            if epoch == 0:
                log_event(
                    ...
                    lr_before=current_lr(),
                    details=(
                        f"held through zero-based epoch {SCHEDULER_START_EPOCH - 1}; "
                        "objective ramp may proceed without LR starvation"
                    ),
                )
        else:
            if not scheduler_release_logged:
                ...
                print(
                    f"[SCHEDULER] Released at epoch {epoch + 1}; monitoring "
