#!/usr/bin/env python3
"""
V5.3-R2.3a Windowed Feasibility Controller

Controller-only fork of V5.3-R2.3_FeasibilityPreservingTransmission.

R2.3a preserves the R2.3 mechanism model, corrected circle-circle geometry,
teacher-relative path budget, detached transmission assignment, smooth class
margins, and checkpoint gates.  It changes only feasibility management:

  * engineering recovery is triggered when a configurable number of recent
    validations are infeasible (default: four of the last six), rather than
    requiring a perfectly consecutive failure streak;
  * constraint multipliers grow only when the same constraint group is
    violated persistently in a moving window, so isolated boundary crossings
    no longer compound multipliers;
  * multiplier growth is gentler, path and point-path caps are lower and
    separate, and stable feasible streaks decay multipliers more quickly;
  * numerical spike detection uses a non-adaptive base objective, so a large
    adaptive weighted loss is not mistaken for a numerical catastrophe; and
  * moving-window controller state is logged and checkpointed for reproducible
    resume/recovery behavior.

No geometry, path, transmission, or deployability gate is changed in this
revision.
"""
...
EXPERIMENT_MODE = "R2.3a"
IS_R1 = True
VARIANT = "V5.3-R2.3a_WindowedFeasibilityController"
PARENT_VARIANT = "V5.3-R2.3_FeasibilityPreservingTransmission"
BASELINE_SOURCE_SHA256 = "f02dd296743bf8da9c17681a76eb0bedaa924db13fb22eefc5e665ef243105d7"
PARENT_SOURCE_SHA256 = "3299c303046ddc7c72f00d3b3f3585575326e9a2d231835a9b440e3c0c2db7ee"
...
parser = argparse.ArgumentParser(
    description=f"{VARIANT}: windowed feasibility-controller experiment",
    formatter_class=argparse.ArgumentDefaultsHelpFormatter,
)
parser.add_argument("--seed", type=int, default=101, help="Master seed for reproducible R2.3a runs")
...
parser.add_argument("--num_points", type=int, default=2000, help="Training target triples")
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Load student model weights only, then start a fresh R2.3a optimizer/run",
)
parser.add_argument(
    "--teacher_model",
    type=str,
...
# R2.3a retains the R1.1 instrumentation and hard-failure thresholds. A dead-gradient recovery
...
def engineering_recovery_window_ready() -> bool:
    observed, infeasible = feasibility_window_counts()
    return (
        observed >= FEASIBILITY_RECOVERY_WINDOW
        and infeasible >= FEASIBILITY_RECOVERY_PATIENCE
    )
...
def recover_engineering_feasibility(
    ...
    """Restore the best deployable model and strengthen violated constraints."""
    ...
    global grace_counter, stagnant_epoch_count, epochs_since_primary_improvement
    global prev_epoch_loss, prev_val_loss, avg_epoch_loss, avg_epoch_distance
    global last_val_metrics, current_state_epoch, last_completed_epoch
    global last_constraint_update_epoch
    ...
    if last_constraint_update_epoch == epoch_index:
        # The persistent-window update already strengthened these groups this
        # epoch.  Do not compound the multiplier a second time merely because
        # the same window also triggered recovery.
        proposed = multiplier_snapshot()
        _, diagnostics = validation_feasibility(metrics)
    else:
    ...
    loaded, metadata = restore_checkpoint_r1(BEST_FEASIBLE_TRANSMISSION_CHECKPOINT)
    if not loaded or metadata is None:
        print("[FEASIBILITY] Failed to restore the best feasible checkpoint.")
        return False

    # Never let a restore undo a multiplier increase requested by the current
    # engineering violation. Preserve the larger value from either state.
    ...
    recovery_lr = max(
        MIN_LR,
        min(live_lr, loaded_lr) * FEASIBILITY_RECOVERY_LR_FACTOR,
    )
    ...
    # Hold both the LR scheduler and repeat engineering restores for the full
    # requested number of *future* epochs.  ``+ 1`` avoids releasing one epoch
    # early when the recovery itself occurs at ``epoch_index``.
    ...
    # Re-evaluate the restored model under the *current* adaptive multipliers.
    # The raw feasibility gates are unchanged, but composite loss and adaptive
    # penalty reporting must not retain stale values from the older checkpoint.
    last_val_metrics = annotate_validation_metrics(evaluate_fixed_validation())

    label = "engineering_feasibility | best feasible transmission"
    log_rollback_event(
        epoch_index + 1,
        pre_train_loss,
        metadata["train_loss"],
    ...
    print(
        f"[FEASIBILITY RECOVERY] Restored state epoch {metadata['epoch']} | "
...
def restore_checkpoint_r1(path: Path | str) -> Tuple[bool, Optional[Dict[str, Any]]]:
    ...
    try:
        ...
        if "optimizer_state_dict" not in checkpoint:
            raise KeyError("optimizer_state_dict is required for coherent R2.3a recovery")
        optimizer.load_state_dict(checkpoint["optimizer_state_dict"])
        ...
        if checkpoint.get("scheduler_state_dict") is not None:
            ...
            print("[WARN] R2.3a checkpoint lacked scheduler state; scheduler recreated.")
...
def rollback_and_recover_r1(
    ...
    if selected is None:
        ...
        print(
            f"[RECOVERY] R2.3a found no eligible checkpoint ({reason}); "
    ...
    if not loaded or metadata is None:
        ...
        print(
            f"[RECOVERY] R2.3a restore failed; applied in-place LR backoff "
    ...
    log_event(
        ...
        lr_after=recovery_lr,
        details=(
            "R2.3a coherent restore: AdamW moments, scheduler, data, sampler/RNG "
            f"restored; no fresh data; plateau cooldown={grace_counter}"
        ),
    )
    print(
        f"[RECOVERY] R2.3a restored {selected['source']} checkpoint "
...
if teacher_source_argument is None:
    raise ValueError(
        "R2.3a requires --teacher_model or --initialize_from_model so a frozen "
...
if teacher_source_variant not in ("V5.3-R2_GeometryFix", VARIANT):
    print(
        "[WARN] The recommended R2.3a teacher is the frozen "
...
if args.initialize_from_model:
    ...
    print(
        f"[INFO] Initialized R2.3a student weights from {initialization_path} "
...
for epoch in range(start_epoch, NUM_EPOCHS):
    ...
    if hard_failure_reason is not None:
        ...
        if recovered:
            continue
        print("[FATAL] R2.3a could not recover from a hard numerical failure.")
        break
    ...
    # R2.3a retains hard-failure protection that bypasses cooldown and all ordinary plateau
    ...
    # Unlike the numerical rollback path, this trigger is based on repeated
    # loss of deployable engineering feasibility inside a moving window.  It
    # catches oscillatory boundary crossings without reacting to one-off noise.
    if (
        ...
        if recovered:
            continue
        print("[FATAL] R2.3a could not restore engineering feasibility.")
        break
    ...
    if not IS_R1:
        ...
        validation_improved = (
            ...
            and prev_validation_stability_metric is not None
            and np.isfinite(prev_validation_stability_metric)
            and np.isfinite(current_validation_stability_metric)
            and current_validation_stability_metric
            < prev_validation_stability_metric - EARLY_STOP_MIN_DELTA
        )
        if train_spike and validation_improved and not validation_spike:
            ...
            print(
                f"[SPIKE CHECK] Base train metric rose "
                f"{prev_train_stability_metric:.6f} -> "
                f"{current_train_stability_metric:.6f}, but base validation "
                f"improved {prev_validation_stability_metric:.6f} -> "
                f"{current_validation_stability_metric:.6f}; no rollback."
            )
            log_event(
                epoch + 1,
                event_label,
